...
class Oneten_Cities_Workflows {
    ...
    public function __construct() {
        if ( !wp_next_scheduled( 'oneten_cities_sync_app_prayer_fuel_hourly' ) ) {
            wp_schedule_event( time(), 'hourly', 'oneten_cities_sync_app_prayer_fuel' );
        }
    ...
    public static function sync_app_prayer_fuel(){
        ...
        foreach ( $campaigns['posts'] as $campaign ){
            //create job
            wp_queue()->push( new DT_Get_Fuel_For_Campaign_Job( $campaign['ID'] ) );
        }
    }
...
use WP_Queue\Job;
class DT_Get_Fuel_For_Campaign_Job extends Job {
    /**
     * @var int
     */
    public $campaign_id;
    ...
    public function handle(){
        ...
        $campaign = DT_Posts::get_post( 'campaigns', $this->campaign_id, true, false );
